/**
 * SASINELWA Email Handler with CloudMailin Support
 * Processes incoming emails from CloudMailin webhook
 */
...
/**
 * Process incoming email - handles both raw email and CloudMailin formats
 */
export async function processEmailWithPDF(emailData) {
  ...
  try {
    ...
    // CloudMailin sends raw email as Buffer or string
    // We need to convert it to a format simpleParser can handle
    let rawEmail = emailData;
    
    // If it's a Buffer, keep it as is
    // If it's a string, convert to Buffer
    if (typeof emailData === 'string') {
      rawEmail = Buffer.from(emailData, 'utf-8');
    } else if (!Buffer.isBuffer(emailData)) {
      // If it's an object (shouldn't be for raw format), stringify and convert
      rawEmail = Buffer.from(JSON.stringify(emailData), 'utf-8');
    }
    
    console.log(`   📧 Email data type: ${typeof emailData}, Buffer: ${Buffer.isBuffer(rawEmail)}`);
